      ...
      init(serviceOpts[name] || {})
        .then(apis => {
          for (const id in apis) {
            ...
            const params = apis[id].toString().match(/\S+\(((?:\S+, )*\S*)\)/)[1].split(', ')
            let schema = null
            ...
            this[id] = (...args) => {
              ...
              if (schema) {
                ...
                if (keys.undefined) {
                  return Promise.reject(new Error(`API ${id} must contain at most ${params.length} parameters`))
                }
                const report = schema.validate(keys)
                if (report.error) {
