        ...
        for (const id in apis) {
          ...
          if (params.length) {
            ...
            if (payloadSchema) {
              ...
              if (!Array.isArray(payloadSchema) && payloadSchema._type === 'binary') {
                // no payload parsing if any of the parameters are buffers
                options.payload.parse = false
                // TODO https://github.com/hapijs/discuss/issues/662
                // options.validate.payload = payloadSchema
              } else {
                try {
                  options.validate.payload = joi.object(arrayToObj(payloadSchema, params)).unknown(false).label(id)
                } catch (exc) {
