...
const {getLogger, extractGroups, getParamNames, arrayToObj, validateParams, isApi}= require('mini-service-utils')
        ...
        const plugin = {
          register: (serv, pluginOpts, next) => {
            ...
            if (Promise.resolve(initialized) !== initialized) {
              return next(new Error(`group ${group} init() method didn't returned a promise`))
            }
                  ...
                  handler: (req, reply) => {
                    ...
                    try {
                      api(...params.map(prop => payload[prop]))
                        .then(res => reply(res).header('x-service-crc', checksum))
                        .catch(err => reply(boom.create(599,  `Error while calling API ${id}: ${err.message}`)))
                    } catch (exc) {
                      // bubble any synchronous problem (not returning promise...)
                      reply(boom.create(599, `Error while calling API ${id}: ${exc.message}`))
                    }
                  }
                })
